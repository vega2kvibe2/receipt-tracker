...
#### 완료 기준
- [x] Upstage API Key로 테스트 호출 시 200 응답을 받는다 ✅ (solar-pro2-251215, 200 OK)
- [x] `pdf2image`로 샘플 PDF 변환이 로컬에서 성공한다 ✅ (Poppler winget 설치, 1654×2339 변환)
- [x] Vercel 서버리스에서 `/tmp` 경로 사용 가능 여부를 공식 문서로 확인한다 ✅ (500MB, 에페머럴)
- [x] LangChain + ChatUpstage 조합의 이미지 전달 예제 코드를 확보한다 ✅ (아래 아키텍처 노트 참조)

> **⚠️ Phase 0 핵심 발견 사항 (2026-05-11)**
>
> 1. **OCR 아키텍처 변경**: PRD 원안의 `document-digitization-vision` Vision LLM 단일 호출 방식 대신, **2단계 파이프라인**이 실제 동작 방식임이 확인됨:
>    - Step 1: `POST /v1/document-digitization` (model=`ocr`) → 텍스트 추출 (신뢰도 0.94+)
>    - Step 2: `ChatUpstage(model="solar-pro3")` + `JsonOutputParser` → 구조화 JSON 반환
> 2. **PDF는 OCR API가 직접 지원**: pdf2image + Poppler 없이 PDF를 직접 `document-digitization` API에 전달 가능 (최대 50MB, 100페이지). pdf2image는 썸네일 표시 용도로만 필요.
> 3. **버전 업데이트 필수**: 아래 requirements.txt 버전이 실제 테스트 완료 버전으로 업데이트됨.
> 4. **Poppler 로컬 경로**: `C:\Users\{USER}\AppData\Local\Microsoft\WinGet\Packages\oschwartz10612.Poppler_...\poppler-25.07.0\Library\bin`
> 5. **ChatPromptTemplate 주의**: system 프롬프트 내 JSON 예시의 `{}` → `{{}}` 이스케이프 필수.
...
python-dotenv==1.2.2
```
> 📝 Phase 0에서 실제 테스트 완료한 버전으로 업데이트됨 (2026-05-11)

#### 완료 기준
- [x] `receipt-tracker` Remote Repository가 생성되어 현재 프로젝트 파일들이 Push 되어 있어야 한다 ✅ (https://github.com/vega2kvibe2/receipt-tracker)
- [x] 가상환경 폴더 venv가 생성되어 있고, 패키지가 설치되어 있다 ✅ (fastapi 0.115.12, langchain-upstage 0.7.7 확인)
- [x] `uvicorn backend.main:app --reload` 실행 시 FastAPI 서버가 정상 기동되는지 확인하고 서버가 스타트 되어 있어야 한다 ✅
- [x] 서버가 잘 동작하는지 테스트 하는 Health Check 기능이 반드시 작성되어 있어야 한다 ✅ (`GET /health` → `{"status":"ok","message":"Receipt Tracker API is running"}`)
- [x] `http://localhost:8000/docs` Swagger UI가 열린다 ✅ (200 OK 확인)
